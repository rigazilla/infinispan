...
public abstract class RespRequestHandler {
    ...
    * <p>
    * The {@link ByteBufPool} provides the means to allocate a
    * {@link io.netty.buffer.ByteBuf} with the required
    * free size. <b>All</b> the writes must utilize the {@link ByteBufPool} instead
    * of manually allocating buffers.
    * </p>
    * <b>Thread-safety</b>: The allocator and the {@link io.netty.buffer.ByteBuf}
    * must be utilized only from the
   ...
   public ByteBufPool allocator() {
      assert ctx.channel().eventLoop().inEventLoop()
            : "Buffer allocation should occur in event loop, it was " + Thread.currentThread().getName();
      return allocatorToUse;
   }

   public final CompletionStage<RespRequestHandler> handleRequest(ChannelHandlerContext ctx, RespCommand command,
         List<byte[]> arguments) {
   ...
   /**
    * Handles the RESP request returning a stage that when complete notifies the
    * command has completed as well as
    ...
    * This ByteBuffer should only have bytes written to it adding up to the size
    * requested.
    * The ByteBuffer itself should never be written to the context or channel and
    * flush should also never be invoked.
    * Failure to do so may cause mis-ordering or responses as requests support
    * pipelining and a ByteBuf may not be
    ...
    * @return stage that when complete returns the new handler to instate. This
    *         stage <b>must</b> be completed on the event loop
    */
   ...
   public CompletionStage<RespRequestHandler> stageToReturn(CompletionStage<RespRequestHandler> stage,
         ChannelHandlerContext ctx) {
      assert ctx.channel().eventLoop().inEventLoop();
   ...
   /**
    * Handles ensuring that a stage TriConsumer is only invoked on the event loop.
    * The TriConsumer can then do things
    * such as writing to the underlying channel or update variables in a thread
    * safe manner without additional
    * synchronization or volatile variables.
    * <p>
    * If the TriConsumer provided is null the provided stage <b>must</b> be
    * completed only on the event loop of the
    * provided ctx.
    * This can be done via the Async methods on {@link CompletionStage} such as
    * {@link CompletionStage#thenApplyAsync(Function, Executor)};
    * <p>
    * If the returned stage was completed exceptionally, any exception is ignored,
    * assumed to be handled in the
    * <b>biConsumer</b> or further up the stage.
    * The <b>handlerWhenComplete</b> is not invoked if the stage was completed
    * exceptionally as well.
    * If the <b>triConsumer</b> or <b>handlerWhenComplete</b> throw an exception
    * when invoked the returned stage will
    ...
    * @param stage               The stage that will complete. Note that if
    *                            biConsumer is null this stage may only be
    *                            completed on the event loop
    * @param ctx                 The context used for this request, normally the
    *                            event loop is the thing used
    * @param biConsumer          The consumer to be ran on
    * @param handlerWhenComplete A function to map the result to which handler will
    *                            be used for future requests. Only ever invoked on
    *                            the event loop. May be null, if so the current
    *                            handler is returned
    * @return A stage that is only ever completed on the event loop that provides
   ...
   private <E> CompletionStage<RespRequestHandler> stageToReturn(CompletionStage<E> stage, ChannelHandlerContext ctx,
      ...
      if (biConsumer != null) {
         // Note that this method is only ever invoked in the event loop, so this
         // whenCompleteAsync can never complete
         // until this request completes, meaning the thenApply will always be invoked in
